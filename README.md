...
DSH 原生已渲染这些信息，但默认仅在 Hover 时显示，容易被错过。本插件以一条
scoped CSS 覆盖将其常驻显示：复用原生渲染与原生位置，日期/时间格式随 DSH
自动更新，不新增任何可见 UI。
...
- DeepSeek Harness（`dsh web` Web 界面）；已在当前 checkout 对应版本（web 客户端包 `0.1.0-rc.5`）验证
