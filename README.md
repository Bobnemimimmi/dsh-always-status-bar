...
一个轻量级 DeepSeek Harness（DSH）Web UI 插件：让聊天消息**原生自带的发送时间戳**（`日期 | 时间`）**无需鼠标悬停即可始终显示**。

> 核心原则：只改变 DSH 原生时间戳的可见性，不重新生成时间戳，不改变消息结构，不修改 DSH 源码。

## 行为

| | 未安装（DSH 原生） | 安装本插件后 |
| --- | --- | --- |
| 用户消息时间 | 悬停到消息气泡后才显示 | 始终显示在原生位置 |
| Assistant 消息时间 | 悬停到消息末尾操作栏后才显示 | 始终显示在操作栏原生位置 |
...
- DSH 客户端构建管线以 `cssModules.pattern = '[hash]_[local]'` 编译 CSS
  ...
  `[class*="timeEnd"]` 能精确命中时钟 span，而不会命中同行里的
  复制 / 分支 / 第三方按钮。
...
2. factory 返回 Cordis 插件形态（`{ name, apply }`），boot Loader 将其作为
   图中的一个 entry 激活；
3. `apply` 注入一个 `<style data-plugin="dsh-always-timestamp">` 标签
...
```sh
dsh plugin --profile web add ./dsh-always-timestamp
```

**GitHub 安装**：

```sh
dsh plugin --profile web add github:<owner>/dsh-always-timestamp
```

git 安装拉取源码并由 pnpm 运行本包的 `prepare`（即 `build`）脚本；
pnpm ≥10 首次会拒绝并打印允许构建的提示，按提示把 `allowBuilds:
dsh-always-timestamp: true` 加入 profile 的 `pnpm-workspace.yaml` 后重跑即可。
`lib/` 已提交，本地目录与 tarball 安装无需任何构建步骤。

安装完成后重启 `dsh web`，加载任意 Session 即可看到时间戳常驻显示。

## 卸载

```sh
dsh plugin --profile web remove dsh-always-timestamp
```

随后重启 `dsh web`：时间戳恢复为仅在 Hover 时显示，不留下 DOM 节点、
...
| B Assistant 消息 | 不悬停操作栏 | 原生 `日期 \| 时间` 已可见 |
| C Hover | 分别悬停气泡与操作栏 | 时间戳保持显示，无闪烁、跳位、重复 |
| D 新消息 | 发送消息并等待回复 | 双方新消息时间戳自动常驻 |
...
插件依赖两个事实：原生时钟容器上的 `data-time-hover-root` 锚点，以及
...
改动了隐藏方式或类名约定，只需调整 `src/client/always-timestamp.css`
中的选择器。
